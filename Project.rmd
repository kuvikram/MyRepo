...
graph3 <- function(name = "India"){
  if(name != ""){
    ...
    print(df_graph)
    #df <- filter(df_spread, tolower(Country) == tolower(name))
    p1 <- qplot(Year, Life, data = df_graph, geom = "point" ) + geom_smooth(stat = 'smooth', color = 'Red', method = 'gam', formula = y ~ s(x, bs = "cs"))
    p2 <- qplot(Year, GDP, data = df_graph, geom = "point" ) + geom_smooth(stat = 'smooth', color = 'Green', method = 'gam', formula = y ~ s(x, bs = "cs"))
    p3 <- qplot(Year, Social, data = df_graph, geom = "point" ) + geom_smooth(stat = 'smooth', color = 'Blue', method = 'gam', formula = y ~ s(x, bs = "cs"))
